...
function gunicornrun(){
  gunicorn -w ${UWSGI_PROCESSES:-2} --timeout ${UWSGI_TIMEOUT:-300} --access-logfile - --error-logfile -  --bind :${UWSGI_PORT:-5000} app:app
}
...
function build(){
  git pull && docker build -t ffmpeg .
}
